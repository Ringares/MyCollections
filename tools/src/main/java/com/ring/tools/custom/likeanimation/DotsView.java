...
import com.ring.tools.utils.ValueUtils;
...
public class DotsView extends View {
    ...
    private void updateDotsAlpha() {
        float progress = (float) ValueUtils.clamp(currentProgress, 0.6f, 1f);
        int alpha = (int) ValueUtils.mapValueFromRangeToRange(progress, 0.6f, 1f, 255, 0);
    ...
    private void updateDotsColor() {
        if (currentProgress < 0.5f) {
            float progress = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0f, 0.5f, 0, 1f);
            ...
            float progress = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, 0, 1f);
    ...
    private void updateOuterDotsPositionAndSize() {
        if (currentProgress < 0.3f) {
            this.currentRadius1 = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0.0f, 0.3f, 0, maxOuterDotsRadius * 0.8f);
        } else {
            this.currentRadius1 = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0.3f, 1f, 0.8f * maxOuterDotsRadius, maxOuterDotsRadius);
        }

        if (currentProgress < 0.7f) {
            this.currentDotSize1 = maxDotSize;
        } else {
            this.currentDotSize1 = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0.7f, 1f, maxDotSize, 0);
        }

    }

    private void updateInnerDotsPositionAndSize() {
        if (currentProgress < 0.3f) {
            this.currentRadius2 = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0, 0.3f, 0, maxInnerDotsRadius);
        } else {
        ...
        if (currentProgress < 0.3f) {
            ...
            this.currentDotSize2 = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0.2f, 0.5f, maxDotSize, 0.3 * maxDotSize);
        } else {
            this.currentDotSize2 = (float) ValueUtils.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, maxDotSize * 0.3f, 0);
        }
    ...
    private void setCurrentProgress(Float value) {
        ...
        updateInnerDotsPositionAndSize();
        updateOuterDotsPositionAndSize();
        updateDotsColor();
        updateDotsAlpha();
